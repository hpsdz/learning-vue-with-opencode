(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.SandboxDocsifyPlugin = factory());
  ...
  const get = (data, strKeys) => {
      ...
      for (const key of keys) {
          const res = data[key];
          if (!isExpectType(res, 'object', 'array'))
              return res;
          data = res;
      }
      return data;
  ...
  function initMiniSandbox(hook) {
      const sandboxOptions = (window.$docsify && window.$docsify.sandboxOptions) || {};
      let options = [];
              ...
              for (const filename in currOption) {
                  ...
                  try {
                      if (regConfig) {
                          ...
                          if (keyStr.indexOf('window.') === 0)
                              keyStr = keyStr.slice(7);
                  ...
                  files[filename] = Object.assign(Object.assign({ type }, fileConfig), { defaultValue: value });
              }
              // 渲染 mini-sandbox
              new window.MiniSandbox(Object.assign(Object.assign({}, sandboxOptions), { el,
                  files }));
          }
          options = [];
      });
  }
  window.$docsify.plugins = [].concat(window.$docsify.plugins, initMiniSandbox);

  return initMiniSandbox;
